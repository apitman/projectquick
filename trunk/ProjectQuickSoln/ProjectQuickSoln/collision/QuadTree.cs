...
namespace CSharpQuadTree
{
    ...
    public class QuadTree<T> where T : class, IQuadObject
    {
        ...
        private readonly DoubleSize minLeafSize;
        ...
        public QuadTree(DoubleSize minLeafSize, int maxObjectsPerLeaf)
        {
        ...
        public QuadTree(DoubleSize minLeafSize, int maxObjectsPerLeaf, bool sort)
        ...
        public void Insert(T quadObject)
        {
            lock (syncLock)
            {
                ...
                DoubleRect bounds = quadObject.Bounds;
                if (root == null)
                {
                    var rootSize = new DoubleSize(Math.Ceiling(bounds.Width / minLeafSize.Width),
                                            Math.Ceiling(bounds.Height / minLeafSize.Height));
                    double multiplier = Math.Max(rootSize.Width, rootSize.Height);
                    rootSize = new DoubleSize(minLeafSize.Width * multiplier, minLeafSize.Height * multiplier);
                    var center = new DoublePoint(bounds.X + bounds.Width / 2, bounds.Y + bounds.Height / 2);
                    var rootOrigin = new DoublePoint(center.X - rootSize.Width / 2, center.Y - rootSize.Height / 2);
                    root = new QuadNode(new DoubleRect(rootOrigin, rootSize));
                }
        ...
        public List<T> Query(DoubleRect bounds)
        {
            lock (syncLock)
            {
        ...
        private void Query(DoubleRect bounds, QuadNode node, List<T> results)
        {
            lock (syncLock)
            {
                if (node == null) return;
        ...
        private void ExpandRoot(DoubleRect newChildBounds)
        {
            lock (syncLock)
            {
                ...
                DoubleRect newRootBounds = new DoubleRect(newX, newY, root.Bounds.Width * 2, root.Bounds.Height * 2);
        ...
        public class QuadNode
        {
            ...
            public DoubleRect Bounds { get; internal set; }
            ...
            public QuadNode(DoubleRect bounds)
            {
                Bounds = bounds;
            ...
            public QuadNode(double x, double y, double width, double height)
                : this(new DoubleRect(x, y, width, height))
            {
